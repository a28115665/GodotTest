...
# Button positions (set in _ready based on viewport)
...
func _detect_mobile() -> void:
	# Detect touch device via OS or display
	var os_name = OS.get_name()
	if os_name in ["Android", "iOS"]:
		is_mobile = true
	elif os_name == "Web":
		is_mobile = _check_web_touch()
	if not is_mobile:
		# Also enable if viewport is small (likely mobile browser)
		var vp_size = get_viewport().get_visible_rect().size
		if vp_size.x <= 1024:
			is_mobile = true

func _check_web_touch() -> bool:
	# On web, assume touch support for now (coi-serviceworker handles this)
	if DisplayServer.is_touchscreen_available():
		return true
	# Default to true on web to support mobile browsers
	return true

func _setup_control_node() -> void:
...
func _handle_touch(event: InputEventScreenTouch) -> void:
	if event.pressed:
		...
		# Check A button
		if event.position.distance_to(btn_a_pos) < button_radius * 1.3:
			a_touch_index = event.index
			_press_action("ui_accept")
			return

		# Check B button
		if event.position.distance_to(btn_b_pos) < button_radius * 1.3:
			b_touch_index = event.index
			_press_action("ui_cancel")
			return

		# Check START button
		if event.position.distance_to(btn_start_pos) < button_radius * 1.5:
...
func _update_dpad_direction(touch_pos: Vector2) -> void:
	...
	# Use 4-direction snapping (like a real D-pad)
	...
	if new_direction != current_direction:
		...
		_press_direction(current_direction)

func _press_direction(dir: Vector2) -> void:
...
func _draw() -> void:
	...
	_draw_text_centered(btn_a_pos, "A")

	# Draw B button
	var b_col = active_color if b_touch_index >= 0 else btn_b_color
	draw_circle(btn_b_pos, button_radius, b_col)
	_draw_text_centered(btn_b_pos, "B")

	# Draw START button
	var start_col = active_color if start_touch_index >= 0 else btn_start_color
	var start_w = button_radius * 2.2
	var start_h = button_radius * 0.7
	draw_rect(Rect2(btn_start_pos.x - start_w / 2, btn_start_pos.y - start_h / 2, start_w, start_h), start_col, true)
	_draw_text_centered(btn_start_pos, "START")

func _draw_arrow(pos: Vector2, dir: Vector2, is_active: bool) -> void:
	var color = Color.WHITE if is_active else dpad_arrow_color
	var size = dpad_radius * 0.2
	var points: PackedVector2Array

	if dir == Vector2.UP:
		points = PackedVector2Array([pos + Vector2(-size, size * 0.5), pos + Vector2(0, -size * 0.5), pos + Vector2(size, size * 0.5)])
	elif dir == Vector2.DOWN:
		points = PackedVector2Array([pos + Vector2(-size, -size * 0.5), pos + Vector2(0, size * 0.5), pos + Vector2(size, -size * 0.5)])
	elif dir == Vector2.LEFT:
		points = PackedVector2Array([pos + Vector2(size * 0.5, -size), pos + Vector2(-size * 0.5, 0), pos + Vector2(size * 0.5, size)])
	elif dir == Vector2.RIGHT:
		points = PackedVector2Array([pos + Vector2(-size * 0.5, -size), pos + Vector2(size * 0.5, 0), pos + Vector2(-size * 0.5, size)])

	draw_colored_polygon(points, color)

func _draw_text_centered(pos: Vector2, text: String) -> void:
	var font = ThemeDB.fallback_font
	var font_size = int(button_radius * 0.7)
	var text_size = font.get_string_size(text, HORIZONTAL_ALIGNMENT_CENTER, -1, font_size)
	draw_string(font, pos - Vector2(text_size.x / 2, -text_size.y / 4), text, HORIZONTAL_ALIGNMENT_CENTER, -1, font_size, Color.WHITE)
